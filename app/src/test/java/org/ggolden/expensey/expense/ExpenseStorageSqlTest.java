...
/**
 * Test the ExpenseService hooked up to the ExpanseStorageSql, through the service.
 */
public class ExpenseStorageSqlTest
{
	// the service
	private static ExpenseService service = null;
	...
	public void setup()
	{
		...
		database.setUrl("jdbc:h2:mem:EvalSettingsDataJDBITest;mode=mysql");
		database.setUser("u");
		...
		Transactor db = new Transactor(new DBIFactory().build(environment, database, "db"));

		ExpenseStorage storage = new ExpenseStorageSql(db);
